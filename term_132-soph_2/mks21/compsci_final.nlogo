...
turtles-own [
  location ;; "insky", "nearclouds", inwater", "inground", or "falling"
  evaporation-ready?
  condensation-ready?
  precipitation-ready? 
]
...
to label-patches 
  ...
  [set what? "sky"]
  ask patches with [pcolor = brown - 0.5 or pcolor = brown or pcolor = green - 0.5 or pcolor = green + 10 or pcolor = yellow - 0.1]
  [set what? "ground"]
...
to condensate 
  if count waterparticles with [condensation-ready? = "yes"] > 40
  [combine-cloud
    combine-water
    kill-cloud] 
end
...
to run-off
  ask raindrops with [ycor <= 1]
  [ifelse pcolor = green + 10 or pcolor = brown or pcolor = green or pcolor = brown - 0.5 or pcolor = yellow - 0.1
    [set breed groundwaters 
      set location "inground"
      set shape "water" 
      set size 0.5 
      set color blue - 1
      if pcolor = green or pcolor = brown - 0.5 or pcolor = yellow - 0.1
    ...
    [ifelse pcolor = white
      [ask turtles-here [freeze]]
      [if what? = "water" [ask turtles-here [ back-to-norm ]]]]
    ]
  ask snowflakes with [ycor <= 1]
  [ifelse pcolor = green + 10 or pcolor = brown or pcolor = green or pcolor = brown - 0.5 or pcolor = yellow - 0.1
    [set breed groundwaters 
      set location "inground"
      set shape "water" 
...
to-report newsize-cloud
  report 1.5 * (count cloudparticles in-radius 2) * random-float ([size] of self) 
end
...
to-report raindrop-count
  report 0.01 * [size] of self * rain-intensity / 10 + 5 
end

to-report snowflake-count
  report 0.01 * [size] of self * snow-intensity / 10 + 5
end 
...
to combine-cloud
  ask cloudparticles [
    ...
    [set condensation-ready? "yes"
      ...
      set color scale-color gray size 30 0
...
to run-stream
  downhill pycor
  if any? neighbors4 with [pcolor = blue + 1]
  [set heading 90
    fd 1  
...
-Use of veriables and reporters in Watercycle (Community Models)
...
-Can't make snows melt as of now.
-The brown tunnel in the bay setup is supposed to show that it collects water and water moves through until it reaches the sea. However, we can only get raindrops to stop moving and head towards the ocean but not move after the condition that any four of its neighbor has a patch color of blue. Therefore, the code directs the water to move immediately forward 15, as of now. 
-Condensation isn't as realistic because sometimes, clouds just disappear from one location to join and form another in a different location.
-Water particles get stuck on mountain, even though the color of mountain and the color of the underground tunnel is different from each other.
-The bright green patches represent a "river" where water particles can flow down into the ocean. However, this could not be achieved. We tried to move the water particles to one of patches with same patch color and to move it downhill using pycor.


##OUR DR RACKET MINI-PROJECT

For the first part of our project we did a Dr Racket function for a given number in a given base convert that number to a different base. We learned that we need a lot of different preliminary functions to define this one functions. Some preliminary functions that we needed are getOnesDigit, getNthDigit, find_power, fromBase10, and getBase10. The thought process was if you want to change between bases, usually you would need to convert the number first into base 10 and from base 10 you would change the base. So first we realize that we need to have functions such as fromBase10 (converting a number from base 10 to a different base) and get base 10 (from a number that is not base 10 to convert it to base 10). If we want these functions then we must use other smaller preliminary functions such as getOnesDigit and getNthDigit. 
