...
;--------------------------------------------------------------
to go
  set temperature temperature + sun-intensity / 10
  
  while [ count waterparticles with [ location = "inwater" ] > 25]
  ...
  ask waterparticles with [pycor <= -3 and pycor > (-3 - evaporation-lvl / 15) and pcolor = blue] 
  [ repeat ( temperature - (temperature mod 5) / 5 ) [
    evaporate
    wait 2 ]
  ]
  ...
  while [count waterparticles with [location = "nearclouds"] >= 25]
    [condensate
      wait 10]
  tick 
  
  if count waterparticles with [location = "nearclouds" ] < 25  and count cloudparticles > 6 
    [precipitate
...
to evaporate  
  ...
  ifelse humidity > random 50 
    [while [ycor <= 11] 
      [set heading random 250 - 150
        fd random 3 + evaporation-rate / 10 - 1
        ]
    set location "insky"
  ...
  if any? waterparticles with [ location = "inwater" ]
  [fd 1]
  
end

to condensate ;;figureout size and color
  ask turtles with [pycor >= 11 and location = "nearclouds"] 
  [ask one-of turtles-here [
    ifelse any? turtles with [location = "nearclouds"] in-radius 2 
      [die
        ask patch-here [sprout-cloudparticles 1
          [ask cloudparticles-here [
            set size 0.5 * (count waterparticles in-radius 2) 
            set color 9.9 - 0.5 * (count waterparticles in-radius 2)]
          ]
        ]
      ]
      [ask cloudparticles with [ycor < 13]
        [travel]]
  ]
  ]
  tick
...
to precipitate 
  ask turtles [
    ask cloudparticles with [ycor > -3 and size > 15]
    [hatch-raindrops ([size] of myself * rain-intensity / 5 + 1)  [ 
      ask raindrops-here [
...
to recycle
  ask patches with [pycor < -3 and pcolor = blue]
  [ask turtles-here 
    [set breed waterparticles
      set color sky - 1
    ]
  ]
  ask patches with [pcolor = green or pcolor = yellow or pcolor = brown]
  [ask turtles-here
    [set breed waterparticles
      set color sky - 1 
